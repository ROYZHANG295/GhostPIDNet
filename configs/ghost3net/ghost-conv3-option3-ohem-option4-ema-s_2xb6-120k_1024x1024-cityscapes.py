# 20260225:
# ROY: 方案三：优化 OHEM（在线难例挖掘）策略	+ 方案四： EMA
...
# 改4
# ==================================================================
# 方案四：开启 EMA (最终修正版 - 无需 import)
# ==================================================================

# 1. 强制引入 mmengine 的 averaging 模块
# 这相当于在后台执行了 import，让注册器能看到 ExponentialMovingAverage
custom_imports = dict(
    imports=['mmengine.model'], 
...
custom_hooks = [
    dict(
        ...
        # 【关键点】
        # 1. 必须用字符串，不能用类对象
        # 2. 加上 'mmengine.' 前缀确保跨库调用
        ema_type='mmengine.ExponentialMovingAverage', 
        momentum=0.0002,
        update_buffers=True,
