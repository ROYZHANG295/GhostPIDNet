...
#         ]
        loss_decode=[
            # -----------------------------------------------------------
            # 1. 辅助 Loss (Aux Head): 监督 I 分支
            # -----------------------------------------------------------
            dict(
                type='CrossEntropyLoss',
                use_sigmoid=False,
                class_weight=None,  # 显式设为 None
                loss_weight=0.4),

            # -----------------------------------------------------------
            # 2. 主 Loss (Main Head): 监督最终输出
            #    开启 OHEM，自动挖掘 Wall/Fence/Pole 等难样本
            # -----------------------------------------------------------
            dict(
                type='OhemCrossEntropy',
                thres=0.9,
            ...
            # -----------------------------------------------------------
            # 3. 边界 Loss (Boundary Head): 监督 D 分支
            #    权重从 20.0 降为 10.0，适应 Ghost 模块较弱的边缘特征
            # -----------------------------------------------------------
            dict(
                type='BoundaryLoss', 
                loss_weight=10.0),  # ▼▼▼ 关键降权 ▼▼▼

            # -----------------------------------------------------------
            # 4. 边界感知语义 Loss (Boundary-aware Semantic): PIDNet 特色
            #    利用边界修正语义，稍微降权以减少噪声影响
            # -----------------------------------------------------------
            dict(
                type='OhemCrossEntropy',
                thres=0.9,
                min_kept=100000,
                class_weight=None,  # ▼▼▼ 去掉 class_weight ▼▼▼
                loss_weight=1.0)    # 保持 1.0 或微调为 0.8 均可
        ]
        ),
