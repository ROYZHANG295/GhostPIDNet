...
model = dict(
    ...
    decode_head=dict(
        type='DDRHeadHALOAvg3OptAblation',
        in_channels=32 * 4,
        channels=64,
        ...
        # =========================================================
        # 🔬 消融实验 1：仅验证 Laplacian Boundary 的有效性
        # =========================================================
        use_laplacian=True,             # ✅ 开启：使用拉普拉斯算子提取纯净边缘
        use_dynamic_dilation=False,     # ❌ 关闭：退化为全程固定 Dilation=3
        use_3stage_schedule=False,      # ❌ 关闭：退化为全程固定权重 dice_w=1.0, fb_w=1.0
        use_halo_feedback=False,        # ❌ 关闭：切断跨分支反哺，只做普通的边界辅助Loss
