...
model = dict(
    ...
    decode_head=dict(
        ...
        align_corners=True,
        loss_decode=[
        # 1. 主分支：使用 120k 带有预热机制的 Edge-Aware OHEM
        # dict(
        #     type='EdgeAwareOhemCrossEntropy', 
        #     thres=0.9,
        #     min_kept=131072,
        #     target_edge_weight=2.0,   # 最终达到的权重
        #     delay_iters=15000,        # 前 15k 不加权
        #     warmup_iters=40000,       # 15k 到 40k 之间线性增长到 2.0
        #     loss_weight=0.4),         # 保持 0.4 不变

        # 1. 主分支：使用 20k 缩放版的预热配置
        dict(
            type='EdgeAwareOhemCrossEntropy', 
            thres=0.9,
            min_kept=131072,
            target_edge_weight=2.0,   # 目标边缘权重
            delay_iters=4000,         # 前 4000 步不加权 (权重为1.0)
            warmup_iters=10000,       # 4000 到 10000 步之间线性增长到 2.0
            loss_weight=0.4),         # 【关键】PIDNet 主分支权重保持 0.4
