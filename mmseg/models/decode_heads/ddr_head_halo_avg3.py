...
class DDRHeadHALOAvg3(BaseDecodeHead):
    """
    ===========================================================================
    ...
    3. 创新 2 (全局均分三阶段课程学习 Holistic Average 3-Stage Curriculum)：
       【彻底解决后期震荡】摒弃人为硬编码！引入全局三阶段平均分割调度 (33.3%-33.3%-33.3%)。
       - 阶段 1 (0~33.3%): 强先验期 (Dilation=5, Weight=3.0)，严苛打底。
       - 阶段 2 (33.3%~66.7%): 平滑衰减期 (Dilation=4, Weight线性衰减)，防梯度休克。
       - 阶段 3 (66.7%~100%): 瞬间卸下沙袋，极速冲刺期 (Dilation=3, Weight=0.5)。
         在极低学习率下提供长达 33.3% 的漫长微调期，彻底抚平收敛震荡，冲击 SOTA！
    ===========================================================================
    """
    ...
    # 的“容量悖论”和训练后期的“震荡崩溃”。该调度器通过自动计算三等分点，管理两个关键超参数：
    ...
    def _build_avg3_schedule(self, max_iters: int) -> dict:
        ...
        schedule = {
            # 阶段 1 (0 -> 33.3%)：强先验期。严苛打底。
            0:      {'dilation': 5, 'dice_w': 3.0},
            t1:     {'dilation': 5, 'dice_w': 3.0},
            
            # 阶段 2 (33.3% -> 66.7%)：平滑衰减期。防梯度休克。
            t1 + 1: {'dilation': 4, 'dice_w': 3.0}, 
            t2:     {'dilation': 4, 'dice_w': 1.0},
            
            # 阶段 3 (66.7% -> 100%)：瞬间减负微调期。
            # 留下长达 33.3% 的无突变微调期，彻底抚平后期的曲线震荡！
