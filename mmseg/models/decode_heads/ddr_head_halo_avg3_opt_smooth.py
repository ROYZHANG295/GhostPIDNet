...
class DDRHeadHALOAvg3OptSmooth(BaseDecodeHead):
    """
    ===========================================================================
    ...
    def _build_avg3_schedule(self, max_iters: int) -> dict:
        t1 = int(max_iters / 3.0)        # 40000
        t2 = int(max_iters * 2.0 / 3.0)  # 80000

        schedule = {
            # ==========================================
            # 阶段1：强先验平稳期 (0 ~ t1) —— 【恒定】
            # 行为：dilation 锁死 5，dice_w 锁死 1.0
            # ==========================================
            0:      {'dilation': 5, 'dice_w': 1.0},
            t1:     {'dilation': 5, 'dice_w': 1.0},
            
            # ==========================================
            # 阶段2：平滑衰减期 (t1+1 ~ t2) —— 【平滑下降】
            # 行为：dilation 突变为 4；dice_w 从 1.0 极其平滑地滑落至 0.5
            # 巧妙之处：t1+1 这里的 dice_w 依然写 1.0，底层会自动和 t2 的 0.5 进行线性插值！
            # 这样就完美消灭了 40k 处的垂直断崖！
            # ==========================================
            t1 + 1: {'dilation': 4, 'dice_w': 1.0},  # <--- 关键修改：这里接住 1.0
            t2:     {'dilation': 4, 'dice_w': 0.5},
            
            # ==========================================
            # 阶段3：极速冲刺期 (t2+1 ~ max_iters) —— 【瞬间卸沙袋】
            # 行为：dilation 突变为 3；dice_w 瞬间从 0.5 砸到 0.1，并锁死微调
            # 目的：保留 80k 处的战术断层，彻底释放算力给主干语义分类
            # ==========================================
