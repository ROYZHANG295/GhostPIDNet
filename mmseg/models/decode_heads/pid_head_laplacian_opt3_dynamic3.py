...
class PIDHeadLaplacianOpt3Dynamic3(BaseDecodeHead):
    ...
    def loss_by_feat(self, seg_logits: Tuple[Tensor],
        ...
        # =================================================================
        # 🛡️ 终极稳健版 3 阶段动态课程学习 (修复突变 + 加入反哺控制)
        # =================================================================
        if current_step < 40000:
            # 阶段一：强势引导期 (0~40K) -> 维持您最初的成功设置
            cur_dilation = 5
            cur_dice_w = 3.0
            cur_thresh = 0.5
            ...
            # 阶段三：末期精细微调 (80K~120K)
            cur_dilation = 3  # 安全地细化边界
            cur_dice_w = 0.5  # 把算力还给语义分支
            cur_thresh = 0.6
            # 【新增】边界最准的阶段，100% 信任边界伪标签，全力拉升边缘细节
            cur_sem_bd_w = 1.0  
        ...
        loss['loss_sem_bd'] = cur_sem_bd_w * self.loss_decode[3](i_logit, sem_bd_label) # cur_sem_bd_w 动态控制
