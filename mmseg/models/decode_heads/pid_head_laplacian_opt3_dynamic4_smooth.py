...
class PIDHeadLaplacianOpt3Dynamic4Smooth(BaseDecodeHead):
    ...
    def _get_dynamic_params(self, current_step: int) -> Tuple[int, float, float]:
        ...
        schedule = {
            # 阶段 A (0~40k): 强先验平稳期 (Strong Prior Plateau)
            # 维持高 Dice 权重和低引导阈值，迫使网络在初期快速建立全局几何拓扑认知。
            0: {
                'step':   {'dilation': 5}, 
                'smooth': {'dice_w': 3.0, 'thresh': 0.50}
            },
            40000: {
                'step':   {'dilation': 5}, 
                'smooth': {'dice_w': 3.0, 'thresh': 0.50}
            ...
            # 阶段 B (40k~80k): 第一次延迟衰减 (First Delayed Decay)
            # 拓扑建立后，Dice权重像滑梯一样平滑下降，防止过度拟合边缘；阈值平滑上升。
            80000: {
                'step':   {'dilation': 3}, 
                'smooth': {'dice_w': 1.0, 'thresh': 0.55}
            },
            
            # 阶段 C (80k~120k): 第二次延迟衰减与精细微调 (Final Fine-tuning)
            # 膨胀系数降为 3 细化边缘，其余参数继续平滑衰减，注意力交还给语义分支。
            120000: {
                'step':   {'dilation': 3}, 
                'smooth': {'dice_w': 0.5, 'thresh': 0.60}
            }
        }
