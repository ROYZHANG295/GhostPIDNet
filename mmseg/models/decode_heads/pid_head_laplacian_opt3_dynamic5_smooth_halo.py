...
class PIDHeadLaplacianOpt3Dynamic5SmoothHALO(BaseDecodeHead):
    ...
    def _get_dynamic_params(self, current_step: int) -> Tuple[int, float, float]:
        ...
        schedule = {
            # ==========================================
            # 阶段一：强先验平稳期 (0 ~ 45,840) —— 【黄金分割点 38.2%】
            # 行为：dice_w 锁死在 3.0，thresh 锁死在 0.5，dilation 极粗(5)。
            # 目的：给网络下“猛药”，强迫其在初期建立极强的全局边界认知。
            # ==========================================
            0:      {'step': {'dilation': 5}, 'smooth': {'dice_w': 3.0, 'thresh': 0.50}},
            45840:  {'step': {'dilation': 5}, 'smooth': {'dice_w': 3.0, 'thresh': 0.50}},
            
            # ==========================================
            # 阶段二：黄金平滑过渡期 (45,840 ~ 74,160) —— 【黄金缓冲带 23.6%】
            # 行为：dilation 保持 5 不变！仅 dice_w 从 3.0 极其缓慢地线性滑落到 1.0。
            # 目的：控制变量！维持空间约束不变，仅平滑释放梯度压力，
            #       完美消除硬切换引发的“梯度休克”。
            # ==========================================
            74160:  {'step': {'dilation': 5}, 'smooth': {'dice_w': 1.0, 'thresh': 0.55}},
            
            # ==========================================
            # 阶段三：极速冲刺微调期 (74,160 ~ 120,000) —— 【黄金分割点 61.8%】
            # 行为 1 (74161步)：发生“双重硬切换”。dilation 瞬间从 5 突变为 3，dice_w 瞬间砍到 0.5。
            # 行为 2 (至120k)：参数再次锁死，不再下降。
            # 目的：制造极致的“弹弓效应”！在找极细边界（难度翻倍）的瞬间，
            #       立刻松开脖子上的绳子（权重减半），彻底释放算力给语义主干，冲击 SOTA！
            # ==========================================
            74161:  {'step': {'dilation': 3}, 'smooth': {'dice_w': 0.5, 'thresh': 0.60}},
